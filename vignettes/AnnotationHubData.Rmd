...
local instance on "MachinName"" and then update the production machine later.
...
```{r}
options("AH_SERVER_POST_URL"="http://<MachinName>:9393/resource")
```



To log in to the database on "MachinName"" use this (you will need the password):
```{}
...
Then copy it down to gamay:

```{}
scp dbdump_2014_11_05_fromProd.sql.gz mcarlson@gamay:~
```
...
#### rerun the (now tested) recipe on gamay to insert the rows into a clean copy of the data.
...
```{}
scp dbdump_2014_11_05_FromGamay.sql.gz ubuntu@annotationhub.bioconductor.org:~
```
